...
function Organisation({ list, setList }) {
  ...
  useEffect(() => {
    ...
    //CLIENT API CALL
    axios.get(`https://wb-temp.afexnigeria.com/WB3/api/v1/client/graph/${id}`, options)
      .then(res => {
        setClient(res.data)
    ...
    axios.get(`https://wb-temp.afexnigeria.com/WB3/api/v1/transaction/summary/${id}`, options)
      .then(res => {
    ...
    axios.get(`https://wb-temp.afexnigeria.com/WB3/api/v1/tenant/info/${id}`, options)
      .then(res => {
    ...
    axios.get(`https://wb-temp.afexnigeria.com/WB3/api/v1/stock/position/${id}`, options)
      .then(res => {
        setSummary(res.data)
      }).catch(err => {
        console.log(err)
            ...
            <div className='flex flex-col w-[50%] h-[350px] items-center gap-10'>
              ...
              {userCount &&
                <div className='relative bg-[#FFFF] w-full h-[50%] rounded-xl  flex gap-10 px-8 items-center'>
                  ...
                  <div className=" absolute right-8 top-8 flex gap-5 ">
                    <button ><FaLessThan className="h-[18px] text-[#B4B4B0] font-light " /></button>
                    <button onClick={() => {
                      setUserCount(false);
                      setStorageCount(true);
                    }}><FaGreaterThan className="h-[18px]" /></button>
                  </div>
              ...
              {storageCount &&
                <div className='relative bg-[#FFFF] w-full h-[50%] rounded-xl  flex gap-10 px-8 items-center'>
                  ...
                  <div className='font-semibold text-[25px]'>
                    <p className='text-[14px] text-[#B4B4B0] mb-5'>Storage Capacity (MT)</p>
                    <p> <span className='text-[#9FA19C]'>{service.storage_capacity ? service.storage_capacity.total_capacity : '0'} </span>/{service.storage_capacity ? service.storage_capacity.total_utilization : '0'}</p>
                  </div>
                  ...
                  <div className=" absolute right-8 top-8 flex gap-5 ">
                    ...
                    }}><FaLessThan className="h-[18px] font-light " /></button>
                    <button><FaGreaterThan className="h-[18px] text-[#B4B4B0]" /></button>
                  </div>
                    ...
                    summary?.data?.map((item) => {
                      return (
                        <tr class="text-left border-b border-gray-200 hover:bg-[#e3f7ee]">
                          ...
                          <td class="py-4 px-2">
                           <Dropdown/> <span class="font-medium">{item.item_code}</span>
                          </td>
